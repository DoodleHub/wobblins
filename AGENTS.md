...
a Stage 2 Wobblin periodically produces an egg into one of its two egg slots →
...
- `generate_egg(p_player_wobblin_id)` — (renamed from `claim_egg` in the 2026-07-28 egg-slot revamp — see revamp entry 6 above; the production/"claim" naming swap is the same function, not a new one) validates ownership and that the species is Stage 2, that `now() - coalesce(last_egg_claimed_at, created_at) >= egg_cadence_hours`, **and now also that the source Wobblin doesn't already have `essence_config.max_egg_slots` unclaimed eggs** (`select count(*) from eggs where source_wobblin_id = ... and collected_at is null and hatched_at is null`); inserts an `eggs` row for the chain's Stage 0 species and updates the checkpoint. Refuses to produce past the slot cap regardless of how much cadence time has elapsed — the player must claim an egg to free a slot before another can generate.
- `claim_egg(p_egg_id)` — (this name previously belonged to the production RPC above, now called `generate_egg` — don't confuse the two) player-only, moves one of their own unclaimed eggs (`collected_at is null`) out of its slot and into their Collection: sets `collected_at = now()` and `hatch_ready_at = now() + essence_config.egg_hatch_hours`. Freeing the slot is what lets `generate_egg` produce again. Works on a single egg at a time regardless of how many of the (up to `max_egg_slots`) slots are currently filled — there's no requirement to fill both before claiming one.
...
- `evolution_chain_id` groups all 3 stages of a line — elegantly, it's simply the `id` of that line's own stage-0 row (no separate lookup table needed). `sacrifice_wobblin` requires both Wobblins to share this value; `generate_egg` uses it directly as the new egg's `species_id`.
- `egg_cadence_hours` is set only on stage-2 rows (varies by rarity at seed time — common lines shorter, legendary longer) and drives `generate_egg`'s eligibility check (alongside the egg-slot cap — see "Final-Evolution Egg Generation" below).
...
- `last_egg_claimed_at` is only meaningful when the species is stage 2; despite the name (a holdover from when the production RPC was itself named `claim_egg` — see revamp entry 6), it's the egg-*production* checkpoint, not the player-claim one. `generate_egg` reads `coalesce(last_egg_claimed_at, created_at)` as that checkpoint.
...
- **Produced into a slot** (`generated_at` set at insert, `collected_at`/`hatched_at` both null) — up to `essence_config.max_egg_slots` such rows can exist per `source_wobblin_id` at once; `generate_egg` refuses to insert another past that cap regardless of cadence.
...
| `(tabs)/index` (Home) | Player header (avatar + username) + an essence balance chip and daily-claim button (disabled/"Claimed" once `last_daily_essence_claim_date` matches today) + a Summon entry-point icon + a nudges list (unclaimed achievements, eggs still sitting unclaimed in a slot, eggs claimed and past their hatch countdown, pending trade offers — each routes to the relevant tab) + the featured Wobblin card (glowing portrait + Lv./XP bar only, no stats). Silently calls `claim_passive_essence` on every focus (toast if `granted > 0`) alongside the existing player/featured-Wobblin refetches. **No more Active Tasks card** — tasks don't exist. |
...
Only stage-2 (fully evolved) Wobblins can generate eggs, and only for their own chain's stage-0 species (`generate_egg` uses `evolution_chain_id` directly as the new egg's `species_id`), timestamp-gated server-side by `egg_cadence_hours`. **Reshaped into an egg-slot + time-gated-hatch system in the 2026-07-28 revamp** (see revamp entry 6 above), replacing the essence-fed XP-bar hatching from the 2026-07-27 revamp:

- **Production is slot-capped.** A stage-2 Wobblin holds up to `essence_config.max_egg_slots` (default 2) unclaimed eggs at once. `generate_egg` still requires `egg_cadence_hours` to have elapsed, but now *also* refuses to produce a third egg while two sit unclaimed — cadence alone no longer guarantees a new egg. The Wobblin detail screen's Eggs panel shows the fill state (e.g. "1/2 slots") and disables "Produce Egg" once full, alongside a per-slot "Claim" action.
- **Claiming moves an egg out of its slot and starts the hatch clock.** `claim_egg(p_egg_id)` sets `collected_at = now()` and `hatch_ready_at = now() + essence_config.egg_hatch_hours` (a flat one day by default) — this is what frees the slot for the next `generate_egg` call, and is what makes the egg show up as a tile in the Collection tab. A player can claim a single pending egg without needing both slots filled first.
...
Production, claiming, and hatching remain three genuinely distinct steps (previously two — claiming and producing used to be the same action, see revamp entry 6's naming note), surfaced across two screens: production/claiming on the Wobblin detail screen's Eggs panel, hatching on the Collection screen's egg tiles.
