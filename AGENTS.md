...
buys additional Stage 0 Wobblins from a weekly-rotating shop with essence →
lists Wobblins for sale on a global marketplace, or proposes direct Wobblin-for-Wobblin trades with other players.

This document describes what is **actually built**, not an aspirational spec. When adding features, match the patterns described here before introducing new ones. This app has gone through two revamps:
...
The central experience is now: **level up and evolve your own Wobblins by spending essence — earned passively from your featured Wobblin and via a daily claim, or by sacrificing duplicates — then use fully evolved Wobblins to hatch new ones, buy more from a rotating shop, or trade directly with other players.**

Essence is the primary progression currency: it grants XP directly, fills an egg's hatch-progress bar, and buys Stage 0 Wobblins from the weekly shop. Duplicate-sacrifice leveling still exists alongside it as a free (essence-less) alternative. The marketplace and direct-trade system are now the *only* way Wobblins move between players — with groups/tasks gone, ownership only ever changes hands through a purchase or an accepted trade offer. The database and game systems matter more than visuals — the MVP should not require expensive artwork or animation.
...
- **Expo Router**, not React Navigation directly. Routes are files under `src/app/`. `(tabs)` is a route group for the bottom tab navigator. `wobblin/[id]`, `trade/compose`, and `shop` are stack routes pushed on top of the tabs (not inside `(tabs)/`), so the tab bar isn't visible on them and they need their own back button.
...
src/
  app/            # Expo Router routes (screens) — file path = URL path
    ...
    trade/compose # Trade offer composer, pushed stack route
    shop.tsx      # Weekly shop, pushed stack route (linked from Home)
...
Each domain has a `src/supabase/<domain>.ts` file of plain async functions (calling `supabase.from(...)` or `supabase.rpc(...)`) and a matching `src/hooks/use<Domain>.ts` file of React Query hooks that call those functions. `src/hooks/queryKeys.ts` is the single query-key factory — always add new keys there so mutations can invalidate the right queries. Follow this pattern for new features rather than calling `supabase` directly from a screen. Current domains: `players`, `wobblins`, `eggs`, `essence`, `shop`, `trades`.

## Reusable components (actual)

`Button`, `MonsterCard`, `XPBar`, `TraitBadge` (used for element/rarity pills), `TextField`, `LevelUpBanner`, `EvolutionBanner`, `RewardToast` (generic icon/title/subtitle toast — now used for essence/shop/trade toasts too, not just eggs), `EmptyState`, `LoadingScreen`, `Skeleton`, `ComingSoonScreen`. Reuse these instead of building new ad hoc cards/badges/loaders — the new Shop and Trade screens deliberately introduced **no** new primitive components, reusing `MonsterCard`, `Button`, `TextField`, and `EmptyState` throughout. `AchievementTray`, `HexBadge`/`HexIconBadge`, and `StatBar` were deleted along with the achievements/player-level/per-instance-stats systems they supported — don't reintroduce them without a reason to re-add those systems. `MonsterCard`'s level label reads "Lv. N" (not "Level N") — match that phrasing in any new UI that shows a Wobblin's level as a standalone label (full sentences like "Unlocks at level 15" are fine as prose).
...
Row Level Security is enabled on every table (`players`, `wobblin_species`, `player_wobblins`, `eggs`, `essence_config`, `essence_generation_rates`, `wobblin_level_xp_requirements`, `shop_price_by_rarity`, `shop_rotations`, `shop_listings`, `marketplace_listings`, `trade_offers`). Keep it that way for any new table. `player_public_profiles` is a **view**, not a table, and deliberately sits outside normal RLS — see its own section under "Database Schema" and the note under "Security" below before assuming it follows the same rules.
...
- `get_weekly_shop()` — lazy get-or-create per ISO week (`date_trunc('week', now())`); generates 6 random Stage 0 listings priced by `shop_price_by_rarity` the first time anyone requests that week's rotation.
- `purchase_shop_listing(p_listing_id)` — atomic first-buyer-wins purchase; debits essence, inserts a fresh level-1/0-XP `player_wobblins` row.
- `list_wobblin_for_sale(p_player_wobblin_id, p_price_essence)` / `cancel_listing(p_listing_id)` / `buy_listed_wobblin(p_listing_id)` — fixed-price marketplace. A partial unique index (`marketplace_listings_active_wobblin_idx`) enforces one active listing per Wobblin. `buy_listed_wobblin` is atomic first-buyer-wins and transfers ownership the way `review_task` used to (bumps `acquired_at`).
- `propose_trade_offer(p_offered_wobblin_id, p_recipient_id, p_requested_wobblin_id)` / `respond_to_trade_offer(p_offer_id, p_accept)` / `cancel_trade_offer(p_offer_id)` — direct Wobblin-for-Wobblin offers. **`respond_to_trade_offer` re-validates both Wobblins' current ownership at the moment of acceptance** (either side may have sold/traded/sacrificed theirs since the offer was proposed) and, on success, cascades: any other still-pending offer or active marketplace listing referencing either just-moved Wobblin is auto-cancelled. This re-validate-at-finalization design is what makes it safe to *not* enforce a single-active-offer constraint at propose-time.
...
- `acquired_at` is when the **current** owner came to own this row — distinct from `created_at` (when the row was first created). They match at creation (starter pick, egg hatch, shop purchase) but diverge on a marketplace purchase or accepted trade: `buy_listed_wobblin` and `respond_to_trade_offer` (on accept) both set `acquired_at = now()` without touching `created_at` — the same pattern `review_task` used to follow. Anything ordering/displaying "how long has this player had this Wobblin" must use `acquired_at`, not `created_at`.
...
Singleton tuning-config row (`id boolean primary key default true check (id)` trick — exactly one row): `daily_claim_amount` (default 50), `xp_per_essence` (default 1), `egg_hatch_xp_required` (default 500), `passive_accrual_cap_hours` (default 72). SELECT-only for `authenticated`, no client-facing write policy — every value it drives is read inside an RPC, never decided by the client.
...
## shop_price_by_rarity / shop_rotations / shop_listings

Weekly shop tables. `shop_price_by_rarity(rarity text primary key, price_essence int)` seeded `common=100, rare=400, legendary=1600`. `shop_rotations(id, week_start date unique, created_at)` — one row per ISO week (Monday-start), created lazily by `get_weekly_shop()` the first time anyone opens the Shop that week (no `pg_cron` job; `pg_cron` is available in the project but not installed — a clean upgrade path if rotation ever needs to roll over even with zero traffic some week). `shop_listings(id, rotation_id FK, species_id FK, price_essence, purchased_by FK nullable, purchased_at nullable)` — 6 random Stage 0 species per rotation, `purchased_by` set by `purchase_shop_listing`'s atomic first-buyer-wins update.
...
`id, proposer_id FK, offered_wobblin_id FK, recipient_id FK, requested_wobblin_id FK, status ('pending'|'accepted'|'declined'|'cancelled'), created_at, resolved_at nullable`. SELECT policy: only the proposer or recipient can see a given offer. Unlike the old task-reward lock, a Wobblin can be referenced by multiple simultaneous pending offers (and/or an active marketplace listing) at once — see `respond_to_trade_offer`'s re-validation-and-cascade behavior above for why that's safe.
...
| `(tabs)/index` (Home) | Player header (avatar + username) + an essence balance chip and daily-claim button (disabled/"Claimed" once `last_daily_essence_claim_date` matches today) + a Shop entry-point icon + the featured Wobblin card (glowing portrait + Lv./XP bar only, no stats). Silently calls `claim_passive_essence` on every focus (toast if `granted > 0`) alongside the existing player/featured-Wobblin refetches. **No more Active Tasks card** — tasks don't exist. |
...
| `(tabs)/trade` | Two modes via a segmented control: **Listings** (browse the global marketplace, buy; a "Sell a Wobblin" panel to list one of your own at a fixed essence price; a "My Listings" section with cancel) and **Offers** (incoming/outgoing direct trade offers — accept/decline/withdraw; "Propose a Trade" pushes to `/trade/compose`). |
| `/trade/compose` | Pushed route: find another player by exact username, browse their Wobblins to pick what you want, pick one of your own to offer, submit via `propose_trade_offer`. |
| `/shop` | Pushed route (linked from Home's shop icon): the current weekly rotation of 6 Stage 0 Wobblins priced by rarity, with a Buy button per listing (disabled once purchased or if the balance is insufficient). |
...
Essence is earned two ways: a flat **daily claim** (`claim_daily_essence`, gated to once per UTC calendar day) and **passive generation** from the player's featured Wobblin (`claim_passive_essence`, silently invoked on every Home-screen focus), at an hourly rate that's a function of that Wobblin's species stage and level (`essence_generation_rates`), capped at 72 hours of accrual so leaving the app closed for a long stretch doesn't grant unbounded essence. It's spent three ways: directly granting a Wobblin XP (`spend_essence_for_xp`), feeding an unhatched egg's progress bar (`feed_egg_essence`), and buying Stage 0 Wobblins from the weekly shop (`purchase_shop_listing`). All rates/amounts/thresholds live in `essence_config`/`essence_generation_rates`/`shop_price_by_rarity` — placeholder-tunable, not balance-tested.

## Weekly Shop

Every ISO week, 6 random Stage 0 species are offered for essence, priced by rarity (`shop_price_by_rarity`). The rotation is generated lazily — `get_weekly_shop()` creates it the first time anyone requests it that week, rather than via a scheduled job — so it's technically possible (if unlikely) for a week to pass with no rotation generated until someone finally opens the tab. `purchase_shop_listing` is atomic first-buyer-wins.

## Marketplace & Direct Trading

Two independent ways a Wobblin changes hands now that tasks/groups are gone:

- **Fixed-price marketplace** (`list_wobblin_for_sale` / `cancel_listing` / `buy_listed_wobblin`): list one of your own Wobblins at a price you set; any other player can buy it instantly. One active listing per Wobblin (partial unique index), same pattern as the old task-reward-lock index.
- **Direct trade offers** (`propose_trade_offer` / `respond_to_trade_offer` / `cancel_trade_offer`): propose your Wobblin in exchange for a specific Wobblin owned by another player; they accept or decline. Acceptance **re-validates both Wobblins' current ownership at that exact moment** (either side may have sold/traded/sacrificed theirs since the offer was proposed) and, on success, cascades to auto-cancel any other pending offer or active listing referencing either just-moved Wobblin.

Both mechanisms deliberately allow a Wobblin to be listed for sale *and* have pending trade offers on it simultaneously — correctness is enforced by re-validating at the point of finalization (accept/buy), not by restricting what can be proposed up front.
...
- Anything that mutates monster ownership, level/XP, essence balance, or timing-gated rewards (eggs, weekly shop rotation) must go through a Postgres RPC that re-derives values server-side (see the RPC pattern above); never trust client-computed values for these fields.
...
- `get_advisors(type: "security")` flags externally-callable `SECURITY DEFINER` functions but won't catch a missing ownership check inside one, so review new RPCs for an explicit `auth.uid()` check matching `sacrifice_wobblin`/`propose_trade_offer`/etc.
- **`player_wobblins` SELECT is now a single permissive policy open to any authenticated user** (`using (true)`) — replaced the old owner-only + groupmate-task-reward-visibility pair in the 2026-07-27 revamp, since Trade/Shop need any player to browse any other player's Wobblins to buy/trade. Safe because there are no per-instance stats to leak (see Product Philosophy) — mutations remain fully RPC-gated regardless of read visibility.
- **`players` itself is still owner-only for SELECT** — the old groupmate-visibility policy was dropped along with groups. `player_public_profiles` (a view exposing only `id`/`username`/`avatar`/`created_at`) is what Trade/Shop use instead, specifically so `essence_balance` never gets broadcast globally the way a cosmetic column safely could be.
...
Guilds, breeding, PvP matchmaking, real-time multiplayer, complex animations. **Trading and a marketplace are no longer out of scope** — both were built in the 2026-07-27 revamp (see "Marketplace & Direct Trading" above); if you see old references to trading/marketplace as excluded, they predate that revamp.

---

# Current Status / Suggested Next Steps

Already working: auth, onboarding (with persisted username + avatar), starter selection, the essence economy (daily claim + passive generation from the featured Wobblin, spend-for-XP), duplicate-sacrifice leveling (single or multi-select) under the new level-relative XP curve, evolution through 3 stages, final-evolution egg generation with essence-fed hatching (claim + feed + hatch as distinct steps), a weekly-rotating shop, a global fixed-price marketplace, direct Wobblin-for-Wobblin trade offers, a Collection screen with an eggs section, and a real Profile screen.
...
- **Weekly shop rotation is lazy, not cron-driven** — `pg_cron` is available in the project but not installed; an easy upgrade path if traffic-dependent rotation generation ever becomes a real problem (see "Weekly Shop" above).
- **No essence-economy balancing has been play-tested** — every rate, price, and threshold (`essence_config`, `essence_generation_rates`, `shop_price_by_rarity`, `wobblin_level_xp_requirements`) is placeholder-tunable, matching the project's existing convention for `sacrifice_wobblin`'s XP formula.
- **Marketplace/trade allow a Wobblin to be simultaneously listed for sale and offered in multiple pending trades** — deliberately, resolved by re-validating ownership at accept/buy time rather than a single-active constraint (see "Marketplace & Direct Trading" above).
